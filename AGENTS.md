...
Installs and configures [Polybar](https://github.com/polybar/polybar), a fast and easy-to-use status bar. On Arch Linux, installs from official repos via `pacman`. On Debian/Ubuntu, builds or installs from apt. Deploys `~/.config/polybar/config` and `~/.config/polybar/launch.sh` from templates.
...
- `tasks/archlinux.yml` — installs via `pacman`
...
- **Molecule**: Arch Linux and Ubuntu via Docker
